...
void InitGPIO(void) {
  ...
  InitGPIOPin(LIGHT_PORT, LIGHT_PIN, GPIO_Speed_50MHz, GPIO_Mode_Out_PP);
  GPIO_WriteBit(LIGHT_PORT, LIGHT_PIN, Bit_RESET);

  InitGPIOPin(FRESHNER_PORT, FRESHNER_PIN, GPIO_Speed_50MHz, GPIO_Mode_Out_PP);
  GPIO_WriteBit(FRESHNER_PORT, FRESHNER_PIN, Bit_RESET);

  InitGPIOPin(BUTTON_PORT, BUTTON_PIN, GPIO_Speed_50MHz, GPIO_Mode_IPU);

  InitGPIOPin(SWITCH_PORT, SWITCH_PIN, GPIO_Speed_50MHz, GPIO_Mode_IPU);

  InitGPIOPin(AUX_INPUT_1_PORT, AUX_INPUT_1_PIN, GPIO_Speed_50MHz, GPIO_Mode_IPU);
}
...
void SetLight(uint8_t set) {
  GPIO_WriteBit(LIGHT_PORT, LIGHT_PIN, set ? Bit_SET : Bit_RESET);
}

void SetFreshner(uint8_t set) {
  GPIO_WriteBit(FRESHNER_PORT, FRESHNER_PIN, set ? Bit_SET : Bit_RESET);
}
...
uint8_t GetInput(uint16_t input) {
  switch (input) {  // NOLINT(hicpp-multiway-paths-covered)
    ...
    case 2: {
      return !GPIO_ReadInputDataBit(AUX_INPUT_1_PORT, AUX_INPUT_1_PIN);
    }
  }
  return 0;
}
